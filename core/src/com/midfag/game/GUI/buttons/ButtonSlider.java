...
public class ButtonSlider extends Button {
	...
	public void after_draw()
	{
		...
		float shake=0;
		if (is_overlap()&&(InputHandler.but==0)) {shake=InputHandler.dx/5f;}
	...
	public void some_update(float _d)
	{
		...
		if (hold)
		{
			if (Gdx.input.isKeyPressed(Keys.SHIFT_LEFT)) {mul=10f;}
			if (Gdx.input.isKeyPressed(Keys.CONTROL_LEFT)) {mul=0.1f;}
			
			if ((value+InputHandler.dx*step*mul>min)&&(value+InputHandler.dx*step*mul<max)) {anim+=InputHandler.dx*0.5f;}
			if (anim<0) {anim=4;}
			if (anim>4) {anim=0;}
			
			value+=InputHandler.dx*step*mul;
